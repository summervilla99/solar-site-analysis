...
def geocode_address(addr):
    for addr_type in ["road", "parcel"]:
        ...
        try:
            st.write(f"📡 [요청 중] 주소: `{addr}`, 타입: `{addr_type}`")
            ...
            if data["response"]["status"] == "OK":
                point = data["response"]["result"]["point"]  # ✅ 리스트 아님!
                x = float(point["x"])
                y = float(point["y"])
                st.success(f"🎯 좌표 변환 성공: {addr} → 위도 {y}, 경도 {x}")
                return [y, x]
            else:
                st.warning(f"🚫 변환 실패: {data['response']['status']}")
...
# ------------------ 주소 입력 ------------------
addr = st.text_input("📍 주소를 입력하세요 (예: 서울특별시 종로구 세종대로 110)", "")
if addr:
